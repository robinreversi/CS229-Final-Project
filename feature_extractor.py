...
def featureExtractor(raw_data, filename, lower=0, upper=20000, TF='regular', verbose=0):
    '''
...
if len(sys.argv) > 1:
    ...
    featureExtractor(train_data, strain, lower, upper, TF)
    featureExtractor(dev_data, sdev, lower, upper, TF)
    featureExtractor(test_data, stest, lower, upper, TF)

else:
    featureExtractor(train_data, 'train_data')
    featureExtractor(train_data, 'dev_data')
    featureExtractor(test_data, 'test_data')
